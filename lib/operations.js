...
const { healthcheckUrl } = require("./operations-delivery");
...
function launchConfiguration(env = process.env, { paid = false } = {}) {
  ...
  // An email address can be registered; a Supabase user id cannot. With
  // "Confirm email" off every signup is auto-confirmed, so ADMIN_EMAILS alone
  // hands staff powers — unlimited stories, Story Studio, the publisher's
  // ledger — to whoever claims the address first.
  check(
    "Staff bound to user ids",
    !(env.ADMIN_EMAILS || "").trim() && (env.ADMIN_USER_IDS || "").split(",").map(x => x.trim()).filter(Boolean).every(
      id => /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(id),
    ),
    "Staff access is bound to Supabase user ids, not claimable email addresses.",
  );
  check("Report hash salt", (env.REPORT_HASH_SALT || "").length >= 32, "Abuse-report identifiers use a unique salt.");
  // Unset, the server mints a random secret per boot, so every redeploy voids
  // outstanding teasers against their seven-day TTL: a visitor who read chapter
  // one and comes back to sign up loses the story we already paid to generate.
  check(
    "Teaser signing secret",
    env.TEASER_ENABLED !== "1" || (env.TEASER_SECRET || "").length >= 32,
    "Anonymous teasers stay redeemable across a redeploy.",
  );
  check(
    "Model cost rates",
  ...
  // ops:check exits non-zero on a critical alert, but a scheduled job's exit
  // code is only ever read by whoever goes looking. Without a destination, a
  // credit-ledger mismatch — which also starts answering readers 503 — notifies
  // no one.
  check(
    "Operations alert destination",
    /^https:\/\//.test(env.OPS_ALERT_WEBHOOK_URL || ""),
    "Critical ledger and generation alerts reach a private endpoint, not just logs.",
    paid ? "fail" : "warn",
  );
  check("Operations heartbeat URL", !env.OPS_HEALTHCHECK_URL || healthcheckUrl(env.OPS_HEALTHCHECK_URL), "Configured heartbeat uses a valid Healthchecks ping URL.");
