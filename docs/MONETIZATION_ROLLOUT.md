# Larkspin monetization rollout

Larkspin's paid-credit system should be enabled only after the database ledger,
...
welcome story, recorded in the ledger with a unique idempotency key. (The
ledger reason is stored as `welcome_novella` — an identifier kept for data
continuity; the reader-facing term is "story".)
...
2. Sign in as a new reader and confirm the counter says `1 story left` and
   `first one included`.
